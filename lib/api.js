...
function handleSetMinerPayoutLevel(urlParts, response){
    ...
    minerSeenWithIPForAddress(address, ip, function (error, keys) {
        ...
        var token_level = level * config.coinUnits;
        redisClient.hset(config.coin + ':workers:' + address, 'minPayoutLevel', token_level, function(error, value){
            if (error){
                response.end(JSON.stringify({'status': 'woops something failed'}));
                return;
            }

            log('info', logSystem, 'Updated payout level for address ' + address + ' level: ' + token_level);
